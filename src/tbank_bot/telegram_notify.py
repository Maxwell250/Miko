...
def format_close_notification(event: "CloseEvent") -> str:
    ...
    lines = [
        f"<b>{emoji} ЗАКРЫТИЕ · {event.mode.upper()}</b>",
        f"<b>{event.ticker}</b> · {_dir_emoji(event.direction)}",
    ...
    if event.structure_summary:
        lines.append(f"\n📐 <i>{event.structure_summary[:200]}</i>")
    return "\n".join(lines)
...
def format_paper_open(report: dict) -> str:
    ...
    lines = [
        f"<b>📋 СИГНАЛ (PAPER)</b>",
        f"<b>{report.get('ticker', '?')}</b> · {_dir_emoji(direction)}",
        f"Лоты: {report.get('lots', 1)} · Вход: {report.get('entry', 0):,.2f}",
        f"ГО ~{report.get('margin', 0):,.0f} ₽ · SL {report.get('sl')} · TP {report.get('tp')}",
        f"Confidence: {report.get('confidence', '-')}%",
    ]
    if report.get("structure_summary"):
        lines.append(f"\n📐 <i>{report['structure_summary'][:300]}</i>")
...
def format_cycle_report(report: dict) -> str:
    ...
    lines = [
        ...
        f"{report.get('risk_reason', report.get('reason', ''))}",
    ]
    if report.get("structure_summary"):
        lines.append(f"📐 {report['structure_summary'][:200]}")
    return "\n".join(lines)
