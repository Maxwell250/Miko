...
class Settings(BaseSettings):
    ...
    min_signal_confidence: float = Field(default=60.0, alias="MIN_SIGNAL_CONFIDENCE")
    default_stop_atr_mult: float = Field(default=1.4, alias="DEFAULT_STOP_ATR_MULT")
    default_tp_atr_mult: float = Field(default=2.5, alias="DEFAULT_TP_ATR_MULT")
    strategy_profile: str = Field(default="range_corridor", alias="STRATEGY_PROFILE")
    moex_main_session_only: bool = Field(default=False, alias="MOEX_MAIN_SESSION_ONLY")

    range_lookback_bars: int = Field(default=48, alias="RANGE_LOOKBACK_BARS")
    range_entry_zone_pct: float = Field(default=22.0, alias="RANGE_ENTRY_ZONE_PCT")
    range_max_adx: float = Field(default=24.0, alias="RANGE_MAX_ADX")
    range_min_width_pct: float = Field(default=0.12, alias="RANGE_MIN_WIDTH_PCT")
    range_max_width_pct: float = Field(default=1.8, alias="RANGE_MAX_WIDTH_PCT")
